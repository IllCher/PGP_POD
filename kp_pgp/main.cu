...
__host__ __device__ uchar4 ray(vec3 pos, vec3 dir, triangle* triangles, vec3 light_source, vec3 light_shade, int n, bool shadows_reflections, int recursion_step) {
    int k_min = -1;
    double ts_min;
    ...
    if (shadows_reflections) {
        ...
        if (recursion_step > 0) {
            ...
            reflections = (reflections * (1.0 - reflection_scale) + to_vec3(ray(pos_tmp, reflection_dir, triangles, light_source, light_shade, n, true, recursion_step - 1)) * reflection_scale);
            result = (reflections * (1.0 - transparency_scale) + to_vec3(ray(pos_tmp, dir, triangles, light_source, light_shade, n, true, recursion_step - 1)) * transparency_scale);
        }
...
//done//
void render_cpu(vec3 pc, vec3 pv, triangle* triangles, uchar4* points, int width, int height, double angle, vec3 light_source, vec3 light_shade, int n, int recursion_step) {
    ...
    for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
            ...
            points[(height - 1 - j) * width + i] = ray(pc, dir, triangles, light_source, light_shade, n, true, recursion_step);
        }
    }
}

__global__ void render_gpu(vec3 pc, vec3 pv, triangle* triangles, uchar4* points, int width, int height, double angle, vec3 light_source, vec3 light_shade, int n, int recursion_step) {
    ...
    for (int i = idx; i < width; i += offsetx) {
        for (int j = idy; j < height; j += offsety) {
            vec3 a = {-1.0 + dw * i, (-1.0 + dh * j) * height / width, z};
            vec3 dir = norm(mult(bx, by, bz, a));
            points[(height - 1 - j) * width + i] = ray(pc, dir, triangles, light_source, light_shade, n, true, recursion_step); //меняем индексацию чтобы не получить перевернутое изображение
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    //MPI_Bcast(&path, 128, MPI_CHAR, 0, MPI_COMM_WORLD);
    ...
    MPI_Bcast(&center1.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&center1.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&center1.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color1.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color1.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color1.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&r1, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&center2.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&center2.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&center2.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color2.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color2.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color2.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&r2, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&center3.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&center3.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&center3.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color3.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color3.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color3.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&r3, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    
    MPI_Bcast(&floor_a.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_a.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_a.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&floor_b.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_b.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_b.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&floor_c.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_c.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_c.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&floor_d.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_d.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&floor_d.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    //MPI_Bcast(&texture_png, 128, MPI_CHAR, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color4.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color4.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&color4.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&light_source.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&light_source.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&light_source.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&light_shade.x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&light_shade.y, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&light_shade.z, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);

    MPI_Bcast(&recursion_step, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Bcast(&smoothing_width, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&smoothing_height, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    for (int i = id; i < frames_count; i += number_processor) {
        ...
        if (cmd == "--gpu") {
            ...
            render_gpu<<<128, 128>>>(pc, pv, gpu_figures, gpu_points_smoothing, smoothing_width, smoothing_height, angle, light_source, light_shade, figures.size(), recursion_step);
            ...
            render_cpu(pc, pv, figures.data(), points_smoothing, smoothing_width, smoothing_height, angle, light_source, light_shade, figures.size(), recursion_step);
